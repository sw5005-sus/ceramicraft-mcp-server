...
async def test_send_push_notification_placeholder():
    with patch(
        "ceramicraft_mcp_server.auth.verify_token", AsyncMock(return_value=MOCK_ADMIN)
    ):
        result = await NOTIFICATION_TOOLS["send_push_notification"](
            _admin_ctx(), 42, "Hello", "World"
        )
...
LOG_TOOLS = _register("ceramicraft_mcp_server.tools.log", "register_log_tools")


@pytest.mark.asyncio
async def test_record_audit_log_placeholder():
    with patch(
        "ceramicraft_mcp_server.auth.verify_token", AsyncMock(return_value=MOCK_ADMIN)
    ):
        result = await LOG_TOOLS["record_audit_log"](
            _admin_ctx(), "test-svc", 1, "SYSTEM", "test"
        )
        assert result["success"] is False


@pytest.mark.asyncio
async def test_query_audit_logs_placeholder():
    with patch(
        "ceramicraft_mcp_server.auth.verify_token", AsyncMock(return_value=MOCK_ADMIN)
    ):
        result = await LOG_TOOLS["query_audit_logs"](_admin_ctx())
        assert result["logs"] == []
        assert result["total_count"] == 0


@pytest.mark.asyncio
async def test_verify_audit_chain_placeholder():
    with patch(
        "ceramicraft_mcp_server.auth.verify_token", AsyncMock(return_value=MOCK_ADMIN)
    ):
        result = await LOG_TOOLS["verify_audit_chain"](_admin_ctx())
        assert result["is_valid"] is False
        assert "failed_log_id" in result
