...
consumed by an LLM and should expose human-readable dollar values while keeping
raw cents for traceability.
"""
...
def with_display_money_fields(data: Any, money_keys: set[str]) -> Any:
    ...
    for key, value in data.items():
        if (
            ...
            normalized.setdefault(f"{key}_display", f"${dollars:.2f}")
        else:
