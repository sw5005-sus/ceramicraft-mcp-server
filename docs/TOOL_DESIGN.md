...
in Singapore dollars. Customer-facing MCP read tools convert those fields to
Singapore-dollar display values and keep raw cents in `<field>_cents` plus
formatted `<field>_display`. Admin write tools continue to send cents unchanged.
